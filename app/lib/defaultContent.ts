...
<p className="lead">
  <mark style="background-color: ${highlightColors[0].rgba}">これまでは、Tailwindで記事、文書、ブログ投稿のスタイルを整えようとすると、タイポグラフィに対する鋭い目と多くの複雑なカスタムCSSを必要とする退屈な作業でした。</mark>
</p>
...
<article class="prose">
  ...
  <p>
    <mark style="background-color: ${highlightColors[2].rgba}" data-comment="これは青色のハイライトとコメントの例です">But a recent study shows that the celebrated appetizer may be linked to a series of rabies cases
    springing up around the country.</mark>
  </p>
</article>
\`\`\`
...
2. <mark style="background-color: ${highlightColors[1].rgba}">本当に最初の理由だけで、それがプラグインの要点です。</mark>
...
| <mark style="background-color: ${highlightColors[3].rgba}">ブレット・ハート</mark> | アルバータ州カルガリー | シャープシューター |
